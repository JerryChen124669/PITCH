...
st.markdown("""
    <style>
    /* 1. MAIN APP: Increase base font size for standard text */
    html, body, [class*="st-"] {
        font-size: 24px !important;
    }
    
    /* 2. MAIN APP: Scale up Headers */
    h1 { font-size: 36px !important; }
    h2 { font-size: 30px !important; }
    h3 { font-size: 24px !important; }
    
    /* 3. SIDEBAR ONLY: Force text, labels, and widgets to be smaller (12px) */
    ...
    /* 4. SIDEBAR HEADERS: Keep them proportional */
    [data-testid="stSidebar"] h1 { font-size: 20px !important; }
    [data-testid="stSidebar"] h2 { font-size: 18px !important; }
    
    /* 5. GLOBAL BUTTON FONT SIZE: Targets ALL buttons (main app and sidebar) */
    button, 
    button p {
        font-size: 18px !important; 
        font-weight: bold !important; 
    }
    /* 6. STEP TABS FONT SIZE: Specifically target the Step 1, Step 2 tabs */
    [data-testid="stTabs"] button, 
    [data-testid="stTabs"] button p {
        font-size: 22px !important; /* Make tabs larger than normal buttons */
        color: #1f77b4 !important;  /* Optional: gives the tabs a nice blue color */
    }
    </style>
...
with tab1:
    if uploaded_file is not None:
        ...
        if "val_results" in st.session_state:
            ...
            ax1.set_title(f"Scatter plot: {trained_type} Performance (r={st.session_state.corr:.4f})")
            ...
            with umap_col1: # LEFT: Measured Groundtruth
                ...
                ax_umap_m.axis('off')
                st.pyplot(fig_umap_m,use_container_width=False)
            ...
            with umap_col2: # RIGHT: Predicted Results
                ...
                ax_umap_p.axis('off')
                st.pyplot(fig_umap_p,use_container_width=False)
...
with tab2:
    ...
    if trained_type == "Multilayers Perception":
        ...
        if 'attr' in st.session_state:
            ...
            st.pyplot(fig_umap_ig,use_container_width=False)
    ...
    if st.session_state.step >= 3:
        ...
        if st.session_state.dt_data is not None:
            ...
            with umap_plot:
                ...
                st.pyplot(fig_umap,use_container_width=False)
